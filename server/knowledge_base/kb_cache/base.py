...
class OpenAICompatibleEmbeddings(Embeddings):
    def __init__(
        ...
        self.api_url = openai_api_base.rstrip("/") + "/embeddings"
        self.proxies = {"http": openai_proxy, "https": openai_proxy} if openai_proxy else None
        self.batch_size = batch_size
        self.request_kwargs = {"encoding_format": "float"}
        if dimensions is not None:
            self.request_kwargs["dimensions"] = dimensions
    ...
    def load_embeddings(self, model: str = None, device: str = None) -> Embeddings:
        ...
        if not self.get(key):
            ...
            with item.acquire(msg="初始化"):
                self.atomic.release()
                if model.startswith("text-embedding-"):  # OpenAI-compatible text embedding models
                    dimensions = os.getenv("OPENAI_EMBEDDING_DIMENSIONS") or os.getenv("DASHSCOPE_EMBEDDING_DIMENSIONS")
                    if dimensions:
                        dimensions = int(dimensions)
                    elif model in {"text-embedding-v3", "text-embedding-v4"}:
                        dimensions = 1024
                    else:
                        dimensions = None
                    embeddings = OpenAICompatibleEmbeddings(
                        model=model,
                        openai_api_key=os.getenv("OPENAI_API_KEY", get_model_path(model)),
