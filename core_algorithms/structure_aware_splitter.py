...
class ChineseRecursiveTextSplitter(RecursiveCharacterTextSplitter):
    ...
    def split_text(self, text: str) -> List[str]:
        # 1. 定义需要保护的正则模式：Markdown代码块 和 LaTeX块级数学公式
        code_pattern = r"(```[\s\S]*?```)"
        math_pattern = r"(\$\$[\s\S]*?\$\$)"
        protect_pattern = f"{code_pattern}|{math_pattern}"

        blocks = []
        
        # 2. 替换函数：将匹配到的特殊块存入列表，并返回占位符
        def repl(match):
            blocks.append(match.group(0))
            return f"\n\n___PROTECTED_BLOCK_{len(blocks)-1}___\n\n"

        # 3. 把原文中的特殊块全部替换为占位符
        text_with_placeholders = re.sub(protect_pattern, repl, text)

        # 4. 调用原生的递归切分逻辑处理替换后的文本
        chunks = super().split_text(text_with_placeholders)

        # 5. 把切分结果中的占位符还原回真实的原始块
        final_chunks = []
        for chunk in chunks:
            for i, block in enumerate(blocks):
                placeholder = f"___PROTECTED_BLOCK_{i}___"
                if placeholder in chunk:
                    # 强行复原，哪怕这个块超过了设定的最大 chunk_size，也保证它是一个不可分割的整体
